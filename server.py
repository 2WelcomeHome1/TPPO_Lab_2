...
def main_page():
    angle, speed = fan.get_fan_info()
    return render_template ('index.html', speed = speed, angle = angle)
...
# Check fan info
@app.route("/get_fan_info", methods = ['GET'])
def check_fan_info():
    angle, speed = fan.get_fan_info()
    message = str ('\nCurrent speed: ' + str(speed) + ' rpm\n' + 'Current angle: ' + str(angle)+ ' deg')
    logging.info (message)
    logging.info ('Get "/get fan info" command')
    print (message)
    return jsonify (str(angle), str(speed))

# Set speed
@app.route("/set_speed", methods = ['POST'])
def set_speed_info():
    speed = request.values ['speed']
    logging.info ('Get "/set speed" command with value ' + str(speed))
    try:
        speed = int(speed)
...
def set_angle_info():
    res = request.values ['angle']
    logging.info ('Get "/set angle" command with value ' + str(res))
    try:
        res = int(res)
        if res <= 180 and res >= 0:
            angle = res
